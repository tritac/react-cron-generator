...
const Cron: React.FunctionComponent<CronProp> = (props) => {
  ...
  const getHeaders = () => {
    return (
      <select
        ...
        onChange={(e) => tabChanged(e.target?.value as HeaderValType)}
      >
        {state.headers.map((d, index) => {
          return (
            <option key={index} value={d} selected={state.selectedTab === d}>
              {translate(d)}
            </option>
          );
        })}
  ...
  return (
    <div className=' '>
      ...
      {props.showResultText && <div className=''>{getVal()}</div>}
